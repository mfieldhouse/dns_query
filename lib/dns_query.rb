...
class DNSQuery
  ...
  def initialize(filename)
    ...
    @zone_file_dir = "../input/zone-files-min"
  ...
  def query
    ...
    ip_addresses = File.read("#{@query}").split
    found_fqdn = []
    output = {}
    ...
    number_of_zone_files = Dir["#{@zone_file_dir}/*"].length
    ...
    Dir["#{@zone_file_dir}/*"].each do |filename|
    ...
    puts "Done! All results output to #{@output}"
    end_time = Time.now
